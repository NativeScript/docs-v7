...
Even that this shor-hand syntax is very handy to assign some very simple functions it is missing some important features of the original syntax.
...
testButton.addEventListener(buttonModule.knownEvents.tap, onTap, this);
```
The only difference here is the third optional parameter which represents the **this** argument which will be used as **event handler** context (closure). In other words if you need a **this** argument in your event handler function you have to use the long syntax otherwise **sender** object is used as this.
...
testButton.off(buttonModule.knownEvents.tap);
```
This will remove all listeners for the tap event of the testButton instance. If there are more than one **event listener** second parameter with the name of the callback function could be set and only the referenced event listener will be removed.

* Remove an event listener with long syntax
...
> There is no syntax to remove event listener via an xml declaration.
...
```
What is important about this event is that it is very critical for entire [data binding](./bindings.md) system to work. In order to take advantage of the data binding mechanism its enough just to create your business object as **successors** of the Observable class. Following example shows how to do that:
...
Using Observable methods **get** and **set** works just great for the **propertyChange** event. In many cases, according to some business logic, there is a need to create a custom event. To fire (raise or emit) an event all we have to do is to Observable.notify() method with proper event data when the action is completed. By proper event data we refer to any **successor** of the [EventData interface](./ApiReference/data/observable/EventData.md). This is the very basic information about an event (its name as **eventName** and instance of the **event sender** as **object**).
...
this.notify(eventData);
```
The barely minimum information needed to raise an event is the eventName - it will be used to execute all **event handlers** associated with this event.
...
myCustomObject.on("myCustomEventName", function(eventData){
  console.log(eventData.eventName + " has been raised! by: " + eventData.sender);
})
```
...
As mentioned working with events is very similar to a radio station. In fact the situation is a bit more complicated. In order to be able to notify the listener, the sender has a pointer to the listener. This means that even that listener object is set to null or undefined it is not eligible for garbage collection, since the sender is alive and has a live reference to listener object. This could result in a memory leak in some scenarios when object lifetime of the sender and the listener are quite different. For example if we have an UI element which creates a lot of another controls every control hook for the event of the parent and after some time child control is released (very similar to list view scrolling). In order to prevent such memory leaks it is a good practice to remove your event listener handler before releasing the listener object. Unfortunately in some cases we could not define the exact time when we should call (off or removeEventListener) functions. Here comes another option in NativeScript framework **weakEvents**.
...
Previous example shows how to attach a **weak event listener** to an observable object instance. A closer look to the **handlePropertyChange** function shows that **text** property of the **this** object is changed when **propertyChange** event is raised (via button tap). It demonstrates how to use the **handlerContext** property - the value of this property is taken as **this** argument inside the **event handler** function.
