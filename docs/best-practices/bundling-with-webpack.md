...
Note that the `./app/app.module.ngfactory` file still does not exist. It will be in-memory generated by the Angular compiler (ngc) during build time. For TypeScript to compile the `main.aot.ts` file we will need explicit declaration in `app/app.module.ngfactory.d.ts`:
...
> Note: If you created your project with `tns create AppName --ng`, it will already be prepared for Ahead-of-Time compilation.
