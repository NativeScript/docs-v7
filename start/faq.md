...
NativeScript framework enables developers to use pure JavaScript language to build native mobile applications running on the major mobile platforms - Apple iOS and Google Android. (Windows Universal platform is in our future plans.) The applications UI stack is built using native UI components and because of that, there are no compromises with the User Experience of the applications.

### How much does NativeScript cost?

NativeScript is open source and FREE. 

However, if you would like to use rich-ui components, you can buy [Telerik UI for NativeScript](http://docs.telerik.com/devtools/nativescript-ui/introduction). <Comment: Is Telerik Marketing using "rich UI" or "rich-ui" to describe the components? I think the first option is correct.> For paid support or more tools, please visit [http://www.telerik.com/platform](http://www.telerik.com/platform#overview).

### What makes NativeScript unique?

NativeScript provides more than 90% code sharing and reuse. All the native APIs are available out of the box; you do not need to know Java or Objective-C.

NativeScript uses the native platform's default rendering and layout engine to display the UI of the application. This means that the applications that use the NativeScript framework are exposing exactly the same UX as natively written applications. With NativeScript, the native APIs are available out of the box.

### Where can I see all the features and capabilities of NativeScript?

You can see a dedicated page on this website - [http://www.nativescript.org/allfeatures](https://www.nativescript.org/allfeatures).

### What NativeScript apps are already in the app stores?

Please see the [NativeScript showcase apps](https://www.nativescript.org/showcases). You can also see the repositories that start with "sample-" prefix in our [GitHub organization](https://github.com/nativescript/) and see the source code. <Comment: I think you mean GitHub repository, not GitHub organization.>

### Why should I spend time learning NativeScript?

NativeScript gives you the capability of building applications that run on iOS and Android using a single codebase. 
...
Moreover, when using NativeScript, you can use any native third-party libraries out of the box. You do not need to use wrappers or do any other actions. The new additions to the native platform are available immediately. 
...
For Android apps you can use Windows, OS X and Linux. However, due certain requirements for iOS development, iOS apps can be developed only on OS X Mavericks or later.
...
### How much time does it take to support new releases from Apple and Google?

New native platform updates are available immediately in the JavaScript layer for the developer to consume. There is no need for additional wrappers, only NativeScript framework should be rebuild to include the new APIs from the native platform. <Comment: The wording of the previous sentence is odd. I think you mean to say "There is no need for additional wrappers. The NativeScript framework should be rebuilt to include the new APIs from the native platform." Please modify to make this accurate.> This way you get access to the latest and greatest features of the native mobile OS, even when they are still in BETA/PREVIEW state.

### Can I use other languages to develop except from JavaScript?

Yes. You can find more information about the supported languages [here]({% slug transpilers %}).
...
Yes. For the moment you can use some JS libraries that do not have dependencies on the browser model. Please see [the work we are doing with the AngularJS team](https://www.nativescript.org/blog/details/angular-2.0-running-in-a-native-mobile-app-using-nativescript). We are working to provide deeper support for third-party JS libraries.

### Can I use JavaScript libraries distributed with npm?

Yes. For the moment you can use only the npm modules, which don't have dependencies on NodeJS APIs. Please see this [GitHub issue](https://github.com/NativeScript/nativescript-cli/issues/362) for information about enabling NPM as package manager for NativeScript projects.
...
Developers can use the NativeScript framework to build the UI using either a declarative approach or plain JavaScript code. NativeScript uses the MVVM design pattern, which enables a clean and easy way to create and reuse the UI. Please see the work we are doing together with the AngularJS team on supporting [Angular 2.0 in NativeScript](https://github.com/NativeScript/NativeScript/issues/103).
...
NativeScript registers some objects for the global context. The simplest way to detect whether your code runs within a NativeScript context is the following check:
...
### How do I check which version of NativeScript my project uses?
...
NativeScript is available as a [npm package](https://www.npmjs.com/package/nativescript). Provided you have a recent Node.js version installed, you need to run `npm install -g nativescript`
...
Start by reading the CONTRIBUTING article: [https://github.com/NativeScript/NativeScript/blob/master/CONTRIBUTING.md](https://github.com/NativeScript/NativeScript/blob/master/CONTRIBUTING.md)
