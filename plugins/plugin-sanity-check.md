...
- What you need to setup sanity checks for your NativeScript plugin?
...
- How to check whether the application that have your plugin installed will be successfully build for Android and iOS?
- How to execute all sanity checks on every code change of your plugin’s repository?

##  What you need to setup sanity checks for your NativeScript plugin?

As we mentioned earlier, NativeScript plugins are building blocks. They work as part of NativeScript application. 
...
> **NOTE**: It is very handy to have the plugin and demo application(s) in same repository. The [NativeScript official plugins seed](https://github.com/NativeScript/nativescript-plugin-seed#plugin-folder-structure) defines this structure so if the plugin is based on it, the plugin’s source and the demo app are nicely organized.
...
Having `tslint.json` on root level allows to use same TSLint rules for demo apps as well adding the same script.

Now the command `npm run ci.tslint` will start static analysis.

## How to check whether your plugin will work in bundled NativeScript application?

Key benefits from bundling a NativeScript app are:
- increase the app start-up time
- decrease the size of the app

Check it out in more details into article [“Using Webpack to Bundle Your Code”](http://docs.nativescript.org/angular/tooling/bundling-with-webpack.html). 
NativeScript plugins should work seamlessly in bundled app. This could be verified by testing the bundled demo apps. In order to enable the app for bundling, there are some configuration required. The entire setup is defined in [“Using Webpack to Bundle Your Code”](http://docs.nativescript.org/angular/tooling/bundling-with-webpack.html). 
...
[demo-angular](https://github.com/NativeScript/nativescript-facebook/tree/doc/demo-angular) app will bring more clarity how to setup WebPack for NativeScript Angular app. 

## How to check whether the application that have your plugin installed will be successfully build for Android and iOS?

Maybe the most important sanity check is to test whether the demo application(s) that have the plugin installed actually can be build. NativeScript support Android and iOS so both platform should be covered. There are lots of options which version of OS to choose. 
...
As a summary, the plugin should be tested in app built for:
...
First things first! Add empty `.travis.yml` file on root level.
...
Each step starts after successful completion of the previous one. In this way if there is a functional error, for example, the entire run will be terminated after the fail of first step and the rest of the steps will not be executed. This behavior is controlled by [Build Stages](https://docs.travis-ci.com/user/build-stages).
...
As we mention earlier the plugin should be sanity check on Android as well as iOS. The Android specific requirements can be defined in `.travis.yml` file in `android` section:
...
Add following snippet at the beginning of `.travis.yml` file:
```
matrix:
  include:
```
...
If everything is configured appropriately these sanity checks will be executed on every code change. The result will looks like this:

![](../img/plugins/travis-ci.png)

The huge benefit for NativeScript plugin authors is that once having this sanity checks set up, You gus can be more confident about the quality of your plugins.
