...
* (Android-only) If the plugin contains .JARs, AndroidManifest.xml and resources, it must contain a valid `include.gradle file`, which describes the dependencies.
* (Android-only) If the plugin contains .AARs it doesn't need AndroidManifest.xml, include.gradle or resources.

### Directory Structure

NativeScript plugins which consist of one CommonJS module, and have no native libraries, might have the following directory structure.
...
NativeScript plugins which consist of multiple CommonJS modules, and have no native libraries, might have the following directory structure.
...
* `platforms\android\res`:  (Optional) This directory contains resources declared by the `AndroidManifest.xml` file. You can look at the folder structure [here](http://developer.android.com/guide/topics/resources/providing-resources.html#ResourceTypes). This folder is not necessary if you are using AAR libraries.
* `platforms\android\include.gradle`: This file modifies the native Android configuration of your NativeScript project such as native dependencies, build types and configurations. For more information about the format of `include.gradle`, see [include.gradle file](#includegradle-specification). This file is not necessary if you are using AAR libraries.
* `platforms\android\MyLibrary.jar`: This is a native android library that makes use of some native api. This file is not necessary if you are using AAR libraries.
* `platforms\android\MyLibrary.aar`: This is a native android library. Furthermore 'aar' bundle is the standard binary distribution of an Android Library Project most commonly used by Android Studio. The main difference between a Jar and a AAR is that AARs include resources such as layouts, drawables etc. This makes it a lot easier to create self-contained components.
